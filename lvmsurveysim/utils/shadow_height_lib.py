...
class vector_test_class():
    ...
    def ray(self, distance=1.0, ra=None, dec=None):
        if ra==None:
            ra = self.ra
        if dec==None:
            dec = self.dec
        
        return(position_from_radec(self.ra, self.dec, distance=distance, epoch=None, t=calculator.t, center=calculator.observatory_topo, target=None)) #, observer_data=LCO_topo.at(t).observer_data))

    def test_jd(self, jd=2459458):
        calculator.update_time(2459458)
        for i, (ra,dec) in enumerate(zip(calculator.coordinates[:,0], calculator.coordinates[:,1])):
            print(ra, self.ray(ra,dec).position.au - calculator.pointing_unit_vectors[i])
...
if __name__ == "__main__":
    ...
    try:
        ...
        calculator.set_coordinates(np.array([ra_deg, dec_deg]))
        test_results[test] = "Pass"
    except:
        test_results[test] = "Fail"
