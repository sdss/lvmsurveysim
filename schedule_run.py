...
def run(targets='./targets.yaml', tile=False, sim=False, plot=False, save=False):
   ...
   Parameters:
   targets: str, path to target list yaml file
   tile: bool, whether to tile the survey or load an existing tile database. 
      Tiling is time consuming, so it is recommended to tile once and load the tile database in subsequent runs.
   sim: bool, whether to run the survey simulation or load an existing simulation for analysis.   plot: bool, whether to plot the survey results
   save: bool, whether to save the plots as pdfs in addition to showing them. 
   Returns:
   sim: Simulator instance containing the survey simulation results. 
   '''
   # Creates a list of targets/
   print('Creating target list ...')
   targets = TargetList(target_file=targets)
   ...
   lco_plan = ObservingPlan(2459945, 2461555+300, observatory='LCO') # baseline 2023, until 30-May-2027
   ...
   if sim:
      ...
      sim.save('LCO_2023_6', overwrite=True) # Save as FITS table

   if plot:
      plot_survey(sim, basename='LCO_2023_6', save=save)

   return sim

def plot_survey(sim, basename='LCO_2023_6', save=False):
   # Plot and print:
